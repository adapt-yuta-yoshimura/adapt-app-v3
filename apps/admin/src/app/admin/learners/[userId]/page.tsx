 ...
 *
 * Figma nodeId: 8351:4
 * レイアウト: パンくず → 戻る → 受講者サマリーカード（アバター+名前+ステータス+メール+ID / 編集・凍結・削除 / 4項目グリッド）→ タブ（受講講座 | 決済履歴）→ テーブル
 */
...
export default function LearnerDetailPage({
  ...
  const [activeTab, setActiveTab] = React.useState<'courses' | 'payments'>(
    'courses',
  );
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <nav className="flex items-center gap-1.5 text-[13px] text-textMuted">
        ...
        <span>/</span>
        <Link href="/admin/learners" className="hover:text-textSecondary">
          受講者管理
        </Link>
        <span>/</span>
      ...
      {/* 受講者サマリーカード（Figma: アバター+名前+ステータス / 編集・凍結・削除 / 4項目グリッド） */}
