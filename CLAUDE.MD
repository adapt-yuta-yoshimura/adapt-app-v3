...
**ケースC：正規配置外に1件検出（旧正規配置の可能性）**
```
⚠️ SoTファイル探索失敗（正規配置外に1件検出）

- 使用ファイル：/docs/[api|design]/[ファイル名]（正規配置）
- 検出ファイル：[パス]（例：/adapt-design-files/[ファイル名]）

【状態】
- SoT正規配置が二重存在しています
- `/adapt-design-files/` は廃止された旧正規配置です

【必要な対応】
- 旧正規配置（/adapt-design-files/）のファイルは「正規配置から除外」されています
- 物理削除は不要（ファイル名そのまま残置）
- リネームは不要（リネーム禁止ルール）
- 探索安全装置により停止します

【重要】
- `/adapt-design-files/` のファイルは参照禁止
- 正規配置は `/docs/` のみ
```
...
- 物理削除は行わない（ファイル名そのまま残置）
- リネームは行わない（リネーム禁止ルール）
- 正規配置としての参照は禁止
- 探索安全装置で「正規配置外」として検出・停止対象とする
