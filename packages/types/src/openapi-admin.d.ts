...
export interface paths {
  ...
  "/api/v1/admin/dashboard/kpi": {
    /**
     * ダッシュボードKPI集計
     * @description 受講者数・講師数・講座数・今月売上と前月比・直近推移を返す
     */
    get: operations["API_ADMIN_22"];
  };
  "/api/v1/admin/dashboard/revenue-chart": {
    /**
     * 売上推移チャートデータ
     * @description 指定期間の売上推移データポイントを返す
     */
    get: operations["API_ADMIN_23"];
  };
  "/api/v1/admin/dashboard/activities": {
    /**
     * 最近のアクティビティ（監査ログ）
     * @description 直近の監査ログをoccurredAt降順で返す
     */
    get: operations["API_ADMIN_24"];
  };
}
...
export interface components {
  schemas: {
    ...
    AuditEvent: {
      ...
      actorGlobalRole: components["schemas"]["GlobalRole"];
      courseId?: string | null;
    ...
    KpiCard: {
      /** @description 現在値 */
      current: number;
      /** @description 前月比（%）。講座数の場合は今月新規件数 */
      change: number;
      /** @description 直近推移（ミニチャート用、直近7データポイント） */
      trend: number[];
    };
    DashboardKpiResponse: {
      learners: components["schemas"]["KpiCard"];
      instructors: components["schemas"]["KpiCard"];
      courses: components["schemas"]["KpiCard"];
      revenue: components["schemas"]["KpiCard"];
    };
    RevenueDataPoint: {
      /** @description X軸ラベル（日付や月名） */
      label: string;
      /** @description 売上金額 */
      value: number;
    };
    RevenueChartResponse: {
      /** @enum {string} */
      period: "7D" | "1M" | "6M" | "1Y";
      dataPoints: components["schemas"]["RevenueDataPoint"][];
    };
...
export interface operations {
  ...
  API_ADMIN_24: {
    ...
    responses: {
      ...
      200: {
        content: {
          "application/json": components["schemas"]["DashboardActivitiesResponse"];
        };
      };
    };
  };
